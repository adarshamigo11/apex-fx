...
function checkVercel() {
  if (commandExists('vercel')) { log(`Vercel CLI installed: ${getCommandOutput('vercel', ['--version'])}`); return true; }
  return false;
}
...
function installVercel(pkgManager) {
  ...
  try { const r = spawnSync(entry[0], entry[1], { stdio: 'inherit', shell: isWindows }); if (r.status !== 0) throw new Error(`Exit code: ${r.status}`); } catch (error) { log(`Installation failed: ${error.message}`); process.exit(1); }
}
function main() {
  log('========================================');
  ...
  log('========================================');
  checkNode();
  if (checkVercel()) { log('Vercel CLI is already installed.'); console.log(JSON.stringify({ status: 'already_installed' })); process.exit(0); }
